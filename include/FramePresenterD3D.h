...
public:
    ...
    */
    FramePresenterD3D(CUcontext cuContext, int nWidth, int nHeight, char* winApp, CUstream stream)
        : cuContext(cuContext), nWidth(nWidth), nHeight(nHeight),winAppName(winApp),m_Stream(stream) {}
    /**
    ...
    */
    virtual bool PresentDeviceFrame(unsigned char *dpBgra,unsigned char *render_result, int nPitch, int64_t delay) = 0;
...
protected:
    ...
    */
    ...
    HWND CreateAndShowWindow(int nWidth, int nHeight,char* winName) {
        ...
        char szAppName[100]="D3D";
        strcpy(szAppName,winAppName);
        // char szAppName[]=winAppName;
        WNDCLASS wndclass;
    ...
    */
    void CopyDeviceFrame(unsigned char *dpBgra, int nPitch) {
        ...
        ck(cuGraphicsMapResources(1, &cuResource, m_Stream));
        CUarray dstArray;
        ...
        m.Height = nHeight;
        //ck(cuMemcpy2D(&m));
        ck(cuMemcpy2DAsync(&m, m_Stream));

        ck(cuGraphicsUnmapResources(1, &cuResource, m_Stream));
        ck(cuCtxPopCurrent(NULL));
    }

    void Direct2Cuda(unsigned char *buffer_surface,int nPitch){
        cuCtxPushCurrent(cuContext);
        cuGraphicsMapResources(1,&cuResource,m_Stream);
        CUarray srcArray;
        cuGraphicsSubResourceGetMappedArray(&srcArray,cuResource,0,0);
        ...
        // m.dstArray=dstArray;
        m.dstDevice=(CUdeviceptr)buffer_surface;
        m.WidthInBytes=nWidth*4;
        m.dstPitch=nPitch?nPitch:nWidth*4;
        m.Height=nHeight;

        cuMemcpy2DAsync(&m,m_Stream);

        // cudaMemcpy(direct_bgra.data,buffer_surface,sizeof(uint8_t)*size_bgra,cudaMemcpyDeviceToHost);

        // uint8_t *buffer_data;
        // size_t numBtyes;
        // cudaGraphicsResourceGetMappedPointer((void**)&buffer_data,&numBtyes,cuResource);

        cuGraphicsUnmapResources(1,&cuResource,m_Stream);
